...
class Builder {
    ...
    function add_input( $label, $args = '', $slug = '' ) {
        ...
        $defaults = array(
            ...
            'options'          => array(),
            'wrap_tag'         => 'div',
            'wrap_class'       => array('ny-serach-box-content clearfix'),
            'wrap_id'          => '',
            'wrap_style'       => '',
            'before_html'      => '',
            'after_html'       => '',
    ...
    function build_form( $echo = true ) {
        ...
        foreach ( $this->inputs as $val ) :
            ...
            if ( $val['request_populate'] && isset( $_REQUEST[ $val['name'] ] ) ) {
                ...
                if ( ! in_array( $val['type'], array( 'html', 'title', 'radio', 'checkbox', 'select', 'select_new', 'submit' ) ) ) {
            ...
            switch ( $val['type'] ) {
                ...
                case 'select':
                    ...
                    break;
                case 'select_new':
                    $end .= '<div class="ny-serach-box-content clearfix">';
                    $end .= '<div class="ny-oa-select not-check-first">';
                    $end .= '<a href="javascript:;" class="ny-select-show"></a>';
                    $end .= '<input class="ny-select-value-input" type="hidden" name="'.$val['name'].'" />';
                    $end .= '<div class="ny-select-list hide">';
                    foreach ( $val['options'] as $key => $opt ) {
                        //key为存数字的情况下，需要将其装换为string,不然 === 的时候匹配不上
...
//                    $end .= '<button class="ny-searchbox-btn" id="searchBtn"></button>';
                    break;
                case 'button':
                    //<button class="ny-searchbox-btn" id="searchBtn"></button>
                    $element = 'button';
                    $end     .= '>';
                    break;
                case 'radio':
